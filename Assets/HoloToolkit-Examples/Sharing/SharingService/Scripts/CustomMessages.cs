...
namespace HoloToolkit.Sharing.Tests
{
    ...
    public class CustomMessages : Singleton<CustomMessages>
    {
        ...
        {
            condition = MessageID.UserMessageIDStart,
            Max
        }
        ...
        private void InitializeMessageHandlers()
        {
            ...
            for (byte index = (byte)TestMessageID.condition; index < (byte)TestMessageID.Max; index++)
            {
        ...
        public void SendCondition(float condition)
        {
            // If we are connected to a session, broadcast our info
            if (this.serverConnection != null && this.serverConnection.IsConnected())
            {
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.condition);
